...
namespace NCore\entity\entities\floating;
        ...
        switch ($name) {
            case "domination":
                foreach (array_keys(Cache::$config["domination"]) as $zone) {
                    if (DominationTask::insideZone($zone, $this->getPosition())) {
                        ...
                        $actual = is_null($actual) ? false : FactionAPI::getFactionUpperName($actual);
            ...
            case "koth":
                if (is_numeric(KothTask::$currentKoth)) {
                    ...
                    $remaining = SanctionAPI::format(KothTask::$currentKoth);
            ...
            case "outpost":
                if (!is_null(Cache::$dynamic["outpost"])) {
                    $remaining = SanctionAPI::format(OutpostTask::$nextReward);
                    $faction = FactionAPI::getFactionUpperName(Cache::$dynamic["outpost"]);
                    ...
                    $remaining = SanctionAPI::format(OutpostTask::$currentOutpost);
