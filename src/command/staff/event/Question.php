    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        switch (mt_rand(1, 3)) {
            case 1:
                ...
                Base::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "Soyez le première à réécrire le code pour gagner §e5k$ §f! Code: §e" . $code);

                self::$currentEvent = 1;
                self::$currentReply = $code;
                break;
            case 2:
                $array = Cache::$config["words_list"];
                $word = $array[array_rand($array)];

                Base::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "Soyez le première à trouver le bon mot pour gagner §e5k$ §f! Mot: §e" . str_shuffle($word));
            ...
            case 3:
                ...
                Base::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "Soyez le première à répondre au calcul pour gagner §e5k$ §f! Calcul: §e" . $number1 . " " . $method . " " . $number2);
