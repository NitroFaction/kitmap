...
class TransferAccount extends BaseCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        $oldPlayer = Base::getInstance()->getServer()->getPlayerByPrefix($old);

        $new = strtolower($args["nouveau"]);
        $newPlayer = Base::getInstance()->getServer()->getPlayerByPrefix($new);
        ...
        if ($oldPlayer instanceof Player) {
            $oldData = Session::get($oldPlayer)->data["player"];
        } else {
            if (is_bool($oldData)) {
        ...
        if ($newPlayer instanceof Player) {
            $newData = Session::get($newPlayer)->data["player"];
        } else {
            if (is_bool($newData)) {
    ...
    {
        if (($player = Base::getInstance()->getServer()->getPlayerByPrefix($username)) instanceof Player) {
            Session::get($player)->data["player"] = $data;
            return;
        }
