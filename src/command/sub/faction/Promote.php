...
class Promote extends BaseSubCommand
{
    public function __construct()
    {
        parent::__construct(Base::getInstance(), "promote", "Promouvoi un membre de sa faction");
    }
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        if ($sender instanceof Player) {
            ...
            $next_rank = FactionAPI::getNextRank($target_rank);

            $sender_position = FactionAPI::getRankPosition($sender_rank);
            $target_position = FactionAPI::getRankPosition($next_rank);
            ...
            Cache::$factions[$faction]["logs"][time()] = "§e" . $sender->getName() . " §fpromote §e" . $args["membre"] . "§f" . $rank_name;
            FactionAPI::broadcastMessage($faction, "§e[§fF§e] §fLe joueur §e" . $args["membre"] . " §fvient d'être promu §e" . $rank_name);
        }
    }
