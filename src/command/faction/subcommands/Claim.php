...
class Claim extends FactionCommand
{
    ...
    public function onNormalRun(Player $sender, Session $session, ?string $faction, array $args): void
    {
        if (!is_null(Cache::$factions[$faction]["claim"])) {
            $sender->sendMessage(Util::PREFIX . "Vous avez déjà un claim. Faites §9/f unclaim§f pour le supprimer");
            return;
        ...
        Cache::$factions[$faction]["logs"][time()] = "§9" . $sender->getName() . " §fa récupéré un nouveau claim";
        Faction::broadcastMessage($faction, "§9[§fF§r§9] §fVotre faction vient de récuperer un nouveau claim");
    }
