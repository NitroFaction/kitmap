...
class Unclaim extends BaseSubCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        if ($sender instanceof Player) {
            ...
            Cache::$factions[$faction]["logs"][time()] = "§e" . $sender->getName() . " §funclaim l'ancien claim §c(ADMIN)";
            Faction::broadcastMessage($faction, "§e[§fF§r§e] §fVotre faction vient de perdre son claim (Supprimé par un admin)");

            $sender->sendMessage(Util::PREFIX . "Vous venez d'unclaim le claim de la faction §e" . $faction);
        }
    }
