    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        if ($sender instanceof Player) {
            ...
            if ($amount < 1 || $amount > 1000) {
                ...
                $sender->sendMessage(Util::PREFIX . "Vous devez encore attendre §9" . Util::formatDurationFromSeconds($session->getCooldownData("xp_bottle")[0] - time()) . " §favant de pouvoir re-créer un coinflip");
                return;
            }

            $item = VanillaItems::EXPERIENCE_BOTTLE();
            $item->getNamedTag()->setInt("xp_bottle", $amount);
            $item->setCustomName("§r§fBouteille d'expérience §9(" . $amount . ")");
            ...
            $sender->sendMessage(Util::PREFIX . "Vous avez crée une bouteille d'expérience avec §9" . $amount . " niveaux §fà l'intérieur");
        }
    }
