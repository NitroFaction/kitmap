    ...
    private function createCoinFlip(Player $player): void
    {
        $form = new CustomForm(function (Player $player, mixed $data) {
            ...
            if (1000 > $price) {
                ...
                return;
            } else if ($price > $session->data["player"]["money"]) {
    ...
    private function confirmationForm(Player $player, string $id): void
    {
        $form = new SimpleForm(function (Player $player, mixed $data) use ($id) {
            ...
            if (!isset(CoinFlip::$coinflip[$id])) {
                ...
                $target = Base::getInstance()->getServer()->getPlayerByPrefix(CoinFlip::$coinflip[$id]["username"]);
            ...
            if (CoinFlip::$coinflip[$id]["price"] > $session->data["player"]["money"]) {
            ...
            if ($players[array_rand($players)] === $player->getName()) {
                Base::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "§e" . $player->getName() . " §fa remporté un coinflip de §e" . OtherAPI::format($price) . " §fpièces contre §e" . $target->getName() . " §f!");
                $session->addValue("money", $_price);
            } else {
                Base::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "§e" . $target->getName() . " §fa remporté un coinflip de §e" . OtherAPI::format($price) . " §fpièces contre §e" . $player->getName() . " §f!");
