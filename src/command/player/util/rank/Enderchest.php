...
class Enderchest extends BaseCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        if ($sender instanceof Player) {
            ...
            if ($session->data["player"]["staff_mod"][0] || $sender->getGamemode() === GameMode::SPECTATOR()) {
                ...
                return;
            } else if (!RankAPI::hasRank($sender, "champion")) {
    ...
    public static function setEnderchestGlass(Player $player, Inventory $inventory): void
    {
        ...
        $slots = RankAPI::getRankValue($rank, "enderchest");
        ...
        for ($i = 1; $i <= 26; $i++) {
            ...
            if ($nbt->getTag("enderchest_slots") && $nbt->getString("enderchest_slots") === "restricted") {
                $inventory->setItem($i, ItemFactory::air());
            }

            if ($slots <= $i) {
                $glass = ItemFactory::getInstance()->get(ItemIds::STAINED_GLASS_PANE, 14);
                $glass->setCustomName(" ");
