...
class PackAPI
{
    public static function openPackUI(Player $player): void
    {
        ...
        $form->setTitle("Pack");
        $form->setContent(Util::PREFIX . "Vous possedez actuellement §e" . $session->data["player"]["pack"] . " §fpack(s)");
    ...
    public static function openPack(Player $player): void
    {
        ...
        Base::getInstance()->getLogger()->info("Le joueur " . $player->getName() . " vient d'ouvrir un pack (ses lots: " . $prize . ")");
        Base::getInstance()->getServer()->broadcastTip(Util::PREFIX . "Le joueur §e" . $player->getName() . " §fvient d'ouvrir un pack !");
    ...
    public static function getItems(): array
    {
        ...
        foreach (array_keys(Cache::$config["partneritems"]) as $pp) {
            if ($pp === "pumpkinaxe" || $pp === "effectdisabler") {
        ...
        $form = new CustomForm(function (Player $player, mixed $data) use ($session) {
            ...
            switch ($data[1]) {
                case 0:
                    ...
                    Base::getInstance()->getLogger()->info("Le joueur " . $player->getName() . " vient d'acheter un pack avec des pièces");
                    break;
                case 1:
                    if (100000 > $session->data["player"]["money"]) {
                    ...
                    Base::getInstance()->getLogger()->info("Le joueur " . $player->getName() . " vient d'acheter un pack avec des gemmes");
                    break;
                default:
                    return;
            }
    ...
    public static function executeInteractPackItem(Player $player, PlayerItemUseEvent $event): bool
    {
        ...
        switch ($type) {
            case 0:
                ...
                Base::getInstance()->getLogger()->info("Le joueur " . $player->getName() . " vient d'utiliser un billet de " . $data . " pièces");
            ...
            case 2:
                ...
                Base::getInstance()->getLogger()->info("Le joueur " . $player->getName() . " vient d'utiliser un papier de " . $data . " pack");
