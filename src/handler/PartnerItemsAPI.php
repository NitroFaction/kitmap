...
class PartnerItemsAPI
{
    public static function executeInteractPartnerItem(Player $player, PlayerItemUseEvent $event): bool
    {
        ...
        if (is_null($item->getNamedTag()->getTag("partneritem"))) {
            return false;
        } else if (OtherAPI::insideZone($player->getPosition(), "spawn")) {
        ...
        if (explode(":", Cache::$config["partneritems"][$name])[4] === "2") {
            return false;
        }
        ...
        switch ($name) {
            case "timewarp":
                if ($session->inCooldown($name)) {
                    ...
                    Base::getInstance()->getScheduler()->scheduleDelayedTask(new ClosureTask(function () use ($player, $session, $position) {
                        if ($player->isOnline()) {
            ...
            case "focusmode":
                if ($session->inCooldown($name)) {
                    ...
                    $hit = $session->data["player"]["last_hit"];
                    ...
                    if (is_null($hit[0])) {
                        ...
                        return true;
                    } else if (!($target = Base::getInstance()->getServer()->getPlayerByPrefix($hit[0])) instanceof Player) {
            ...
            case "ninjastar":
                if ($session->inCooldown($name)) {
                    ...
                    if (!$session->inCooldown("combat") || !($target = Base::getInstance()->getServer()->getPlayerByPrefix($data[1])) instanceof Player) {
                        $player->sendMessage(Util::PREFIX . "Le dernier joueur que vous avez tapé n'est plus en ligne");
                        return true;
                    ...
                    Base::getInstance()->getScheduler()->scheduleDelayedTask(new ClosureTask(function () use ($player, $target, $session) {
                ...
                break;
            case "choosableeffect":
                $menu = InvMenu::create(InvMenuTypeIds::TYPE_HOPPER);
                $menu->setName("ChoosableEffect");

                $deleteItem = false;

                $menu->setListener(InvMenu::readonly(function (DeterministicInvMenuTransaction $transaction) use ($item): void {
                    $player = $transaction->getPlayer();
                    $_item = $transaction->getItemClicked();

                    switch ($_item->getId()) {
                        case ItemIds::BLAZE_POWDER:
                            $_item = self::createItem("strength");
                            break;
                        case ItemIds::IRON_INGOT:
                            $_item = self::createItem("resistance");
                            break;
                        default:
                            return;
                    }
                    ...
                    OtherAPI::addItem($player, $_item);

                    $player->sendMessage(Util::PREFIX . "Vous venez de recevoir l'item " . $_item->getCustomName());
                    $player->removeCurrentWindow();
                }));

                $menu->getInventory()->setItem(1, ItemFactory::getInstance()->get(ItemIds::BLAZE_POWDER)->setCustomName("§r§eForce II"));
                $menu->getInventory()->setItem(3, ItemFactory::getInstance()->get(ItemIds::IRON_INGOT)->setCustomName("§r§6Resistance III"));
    ...
    public static function createItem(string $name): Item
    {
        ...
        list($id, $meta, $lore, $customName) = explode(":", Cache::$config["partneritems"][$name]);
    ...
    public static function executeHitPartnerItem(Player $player, Player $target): void
    {
        ...
        switch ($name) {
            case "antibuild":
                ...
                break;
            case "pumpkinaxe":
                if ($target->getArmorInventory()->getHelmet()->getId() === ItemIds::PUMPKIN) {
                    ...
                    $target->getArmorInventory()->setHelmet(ItemFactory::getInstance()->get(86));

                    Base::getInstance()->getScheduler()->scheduleDelayedTask(new ClosureTask(function () use ($target, $helmet) {
                        if ($target->isOnline()) {
            ...
            case "effectdisabler":
                if ($playerSession->inCooldown($name)) {
                    $player->sendMessage(Util::PREFIX . "Veuillez attendre §e" . ($playerSession->getCooldownData($name)[0] - time()) . " §fseconde(s) avant de réutiliser un effect disabler");
                    return;
                } else {
                    $playerSession->setCooldown($name, 120);
                    $targetSession->setCooldown("_" . $name, 12);

                    $player->sendMessage(Util::PREFIX . "Vous venez d'utiliser un effect disabler sur le joueur §e" . $target->getDisplayName());
                    $target->sendMessage(Util::PREFIX . "Le joueur §e" . $player->getDisplayName() . " §fvient d'utiliser un effect disabler sur vous");
                }
                break;
            case "antipearl":
                if ($playerSession->inCooldown($name)) {
