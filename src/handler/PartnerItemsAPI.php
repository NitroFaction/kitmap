...
class PartnerItemsAPI
{
    public static function executeInteractPartnerItem(Player $player, PlayerItemUseEvent $event): bool
    {
        ...
        if (is_null($item->getNamedTag()->getTag("partneritem"))) {
            return false;
        } elseif (OtherAPI::insideZone($player->getPosition(), "spawn")) {
            $player->sendMessage(Util::PREFIX . "Vous ne pouvez pas utilisé cet item au spawn");
            return false;
        } elseif ($session->inCooldown("_partneritemblocker")) {
        ...
        switch ($name) {
            ...
            case "focusmode":
                if ($session->inCooldown($name)) {
                    ...
                    if (is_null($hit[0])) {
                        ...
                        return true;
                    } elseif (time() - $hit[1] > 60) {
                        $player->sendMessage(Util::PREFIX . "Vous devez avoir tappé un joueur les §e60 §fdernières secondes");
                        return true;
                    } elseif (!($target = Main::getInstance()->getServer()->getPlayerByPrefix($hit[0])) instanceof Player) {
            ...
            case "ninjastar":
                if ($session->inCooldown($name)) {
                    ...
                    if (!$session->inCooldown("combat") || !($target = Main::getInstance()->getServer()->getPlayerByPrefix($data[1])) instanceof Player) {
                        $player->sendMessage(Util::PREFIX . "Le dernier joueur que vous avez tapé n'est plus en ligne");
                        return true;
                    } elseif (15 >= ($data[0] - time())) {
            ...
            case "switchball":
                if ($player->getWorld()->getFolderName() !== "map") {
                    ...
                    return true;
                } elseif ($session->inCooldown($name)) {
    ...
    public static function executeHitPartnerItem(Player $player, Player $target): void
    {
        ...
        if (is_null($item->getNamedTag()->getTag("partneritem"))) {
            return;
        } elseif ($playerSession->inCooldown("_partneritemblocker")) {
        ...
        switch ($name) {
            ...
            case "pumpkinaxe":
                if ($target->getArmorInventory()->getHelmet()->getId() === ItemIds::PUMPKIN) {
                    ...
                    return;
                } elseif ($playerSession->inCooldown($name)) {
