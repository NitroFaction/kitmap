...
class KothTask
{
    ...
    public static function run(): void
    {
        ...
        if (is_null(self::$currentPlayer)) {
            foreach ($players as $player) {
                if (!Session::get($player)->data["staff_mod"][0] && $player->isAlive() && OtherAPI::insideZone($player->getPosition(), "koth")) {
            ...
            $player = Main::getInstance()->getServer()->getPlayerByPrefix(self::$currentPlayer);

            if (!$player instanceof Player || Session::get($player)->data["staff_mod"][0] || !$player->isAlive() || !OtherAPI::insideZone($player->getPosition(), "koth")) {
