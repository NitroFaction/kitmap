    ...
    public static function run(): void
    {
        ...
        if (!is_null(Cache::$data["outpost"])) {
            ...
            if (!Faction::exists(Cache::$data["outpost"])) {
                ...
                foreach ($players as $player) {
                    ...
                    if (!$session->data["staff_mod"][0] && $player->isAlive() && Util::insideZone($player->getPosition(), "outpost") && Faction::hasFaction($player) && $session->data["faction"] !== Cache::$data["outpost"]) {
                        ...
                        if ((time() - self::$spam) > 5) {
                            Main::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "La faction §9" . Faction::getFactionUpperName(self::$currentFaction) . " §fest entrain de capturer l'outpost de la §9" . Faction::getFactionUpperName(Cache::$data["outpost"]) . " §f!");
                            self::$spam = time();
                        }
                        return;
                    }
            ...
            if (0 >= self::$currentOutpost) {
                Main::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "La faction §9" . Faction::getFactionUpperName(Cache::$data["outpost"]) . " §fne possède plus l'outpost !");
            ...
            if (0 >= self::$nextReward && !is_null(Cache::$data["outpost"])) {
                ...
                foreach ($members as $player) {
                    ...
                    $player->sendMessage(Util::PREFIX . "Vous venez de recevoir §9" . $price . " §fgrace à l'outpost");
                }

                Faction::addPower(Cache::$data["outpost"], 8);
                Main::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "La faction §9" . Faction::getFactionUpperName(Cache::$data["outpost"]) . " §fvient de récuperer leurs récompenses grace à l'outpost");
            }
            return;
        }
        ...
        if (is_null(self::$currentFaction)) {
            foreach ($players as $player) {
                ...
                if (!$session->data["staff_mod"][0] && $player->isAlive() && Util::insideZone($player->getPosition(), "outpost") && Faction::hasFaction($player)) {
                    ...
                    if ((time() - self::$spam) > 5) {
                        Main::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "La faction §9" . Faction::getFactionUpperName(self::$currentFaction) . " §fest entrain de capturer l'outpost §f!");
                        self::$spam = time();
                    }
                    return;
                }
        ...
        if (0 >= self::$currentOutpost) {
            Main::getInstance()->getServer()->broadcastMessage(Util::PREFIX . "La faction §9" . Faction::getFactionUpperName(self::$currentFaction) . " §fvient de capturer l'outpost");
