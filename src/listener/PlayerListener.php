...
class PlayerListener implements Listener {
	public function onInteract(PlayerInteractEvent $event) : void {
		...
		} elseif (!$player->isSneaking() && $event->getAction() === PlayerInteractEvent::RIGHT_CLICK_BLOCK && $block->isSameState(VanillaBlocks::ANVIL())) {
			$event->cancel();
			$player->removeCurrentWindow();

			Anvil::openAnvil($player);
		} elseif (!$player->isSneaking() && $event->getAction() === PlayerInteractEvent::RIGHT_CLICK_BLOCK && $block->isSameState(VanillaBlocks::ENCHANTING_TABLE())) {
			$event->cancel();
			$player->removeCurrentWindow();
	...
	public function onUse(PlayerItemUseEvent $event) : void {
		...
		if ($executePack || $executePp) {
			return;
		} elseif ($item->equals(VanillaItems::SNOWBALL())) {
			$event->cancel();
			return;
		}
	...
	public function onConsume(PlayerItemConsumeEvent $event) : void {
		...
		} elseif ($item->equals(VanillaItems::COOKED_FISH())) {
		...
		} elseif ($item->equals(VanillaItems::RAW_SALMON())) {
		...
		} elseif ($item->equals(VanillaItems::COOKED_SALMON())) {
		...
		} elseif ($item->equals(VanillaItems::GOLDEN_APPLE())) {
			$event->cancel();
		}
	}
	...
	public function onBreak(BlockBreakEvent $event) : void {
		...
		if (!Faction::canBuild($player, $block, "break")) {
			...
			$event->cancel();
			return;
		} elseif ($session->data["cobblestone"] === false && ($block->isSameState(VanillaBlocks::COBBLESTONE()) || $block->isSameState(VanillaBlocks::STONE()))) {
			$event->setDrops([]);
		}
