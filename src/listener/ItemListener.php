...
use NCore\Base;
use NCore\command\player\util\rank\Enderchest;
...
class ItemListener implements Listener
{
    ...
    {
        if (Session::get($event->getPlayer())->data["player"]["staff_mod"][0]) {
            $event->cancel();
        }
    }
    ...
    public function onTransaction(InventoryTransactionEvent $event): void
    {
        ...
        $staff = Session::get($player)->data["player"]["staff_mod"][0];
        ...
        foreach ($transaction->getActions() as $action) {
            ...
            if (!is_null($targetItem->getNamedTag()->getTag("menu_item")) || !is_null($sourceItem->getNamedTag()->getTag("menu_item"))) {
                ...
                Base::getInstance()->getLogger()->warning("Dupli: " . $player->getName() . " son inventaire a été clear");
            }
